...
const SCHOOL_LOGO_URL = "https://i.postimg.cc/C5RsRHgV/S-5677128-1.png"; // อัปเดตลิงก์โลโก้ที่ใช้งานได้

// Login
const ADMIN_USERNAME = "admin";
const ADMIN_PASSWORD = "Srd-2568";
...
// ========================================================================
// API ENTRY POINTS (รับ Request จาก GitHub Pages)
// ========================================================================
function doGet(e) {
  // สำหรับทดสอบว่า API ทำงานหรือไม่เมื่อเปิดผ่าน Browser
...
function doPost(e) {
  ...
  try {
    ...
    // ตรวจสอบ Token (รหัสผ่าน) สำหรับ Action ที่ต้องการความปลอดภัย
    if (action !== 'login' && action !== 'getRooms' && action !== 'getEvents') {
      if (params.token !== ADMIN_PASSWORD) {
        return ContentService.createTextOutput(JSON.stringify({ success: false, error: "Unauthorized: ไม่ได้รับสิทธิ์" })).setMimeType(ContentService.MimeType.JSON);
      }
    }

    // แยก Routing ตามคำสั่ง
    if (action === 'login') {
      if (params.username === ADMIN_USERNAME && params.password === ADMIN_PASSWORD) {
        response = { success: true, token: ADMIN_PASSWORD };
      } else {
        response = { success: false, error: "ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง" };
      }
    } 
...
// ========================================================================
// PDF REPORT GENERATOR (ฉบับเต็มของเดิม)
// ========================================================================
function generateReportAsPdf(reportType, filterParam) {
  try {
    ...
    // แปลงไฟล์เป็น PDF และเข้ารหัสแบบ Base64 คืนกลับไปยังฝั่ง Frontend
